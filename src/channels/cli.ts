...
import { parsePollBody } from "./poll.ts";
import {
...
const KINDS_HELP =
  ...
  "  poll      — structured-choice question (votes / approvals / decisions); a NEW kind, NOT free-form question. Trust the SHAPE; the option set is an author-claim. Body: JSON conforming to PollBody (see src/channels/poll.ts).\n" +
  "  nudge     — directive wake-signal (dashboard Nudge / Check-comms): wake a hung/idle peer to check the channel. A NEW kind, NOT free-form note/question. Trust verbatim (no schema). Body: free-form mode prose (`working?` / `check your channel — directed from dashboard`).\n" +
  "\n" +
    ...
    switch (cmd) {
      ...
      case "send": {
        ...
        if (targetPlanRef !== null) {
          ...
          if (
            "target_pr" in parsedBody ||
            "target_plan" in parsedBody ||
            "target" in parsedBody
          ) {
            die(
              ctx,
              `--target-plan conflicts with a body that already carries a wire target field or internal target — remove the flag or the body field`,
              { code: 2, category: "VALIDATION" },
            );
          }
          body = JSON.stringify({
        ...
        // audit-ask above). Body must JSON.parse into the PollBody shape per
        // src/channels/poll.ts. Send-time validation gives operators
        // immediate feedback rather than dropping silently on a downstream
        // parse failure.
        if (kind === "poll" && parsePollBody(body) === null) {
          die(
            ctx,
            `[send] poll body failed schema validation — body must be JSON conforming to PollBody (see src/channels/poll.ts + docs/conventions/message-kinds-and-verification.md). Required fields: kind_version=1, question (non-empty string), options (>=2 items, each {id (unique non-empty string), label (non-empty string), description? (string)}); optional multi_select / free_text (booleans, default false).`,
            {
              code: 2,
              category: "VALIDATION",
              remediation:
