...
export function recordCheckTiming(
  ...
  try {
    ...
    // `extractValidSessionId` rejects path-traversal AND log-injection at the
    // API boundary (newlines/CR would corrupt the JSONL parser on read).
    // Sub-step 0.10 RE-2 / L:768.
    const sessionId = extractValidSessionId(rawInput);
