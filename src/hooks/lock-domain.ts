...
export const BUNDLED_LOCK_DOMAINS_BY_EVENT = {
  ...
  "session-start": [
    ...
    },
    {
      ...
      comment:
        'Anchor-pin (REV 0.2 ARCH-1): `setSentinelDotfilesRoot({sessionId, dotfilesRoot})` writes per-(canonical-claude-home, sessionId) heartbeat with `dotfilesRoot` field via writeAtomic UNCONDITIONALLY (before existsSync early-return). Also force-creates `meta.json` via `writeMetaIfMissing` (linkSync-atomic). Provisions per-session worktree: spawns `git worktree add -b worktree/<sid-prefix> <dotfilesCanonical>-<sid-prefix>` — git serializes via its own internal `.git/worktrees/` lockfiles. **P0 substrate canary (backlog L:892, 2026-05-17, Path B fold):** composes `linkCanonicalNodeModules(canonicalPath, worktreePath)` after worktree materialization — `symlinkSync(<canonical>/node_modules, <worktree>/node_modules, "dir")` is filesystem-atomic; EEXIST race recovery re-probes and returns `already-linked` if the racer\'s result matches. Refuses to overwrite operator collisions (real dir, different-target symlink). Multi-source appendPresenceFailure on incomplete/failed/link-failed paths. C1 S2: also calls `recordSessionOsPid(sessionId, osPid)` → read-merge-write of the SAME per-(canonical-claude-home, sessionId) anchor heartbeat via writeAtomic, adding the optional `sessionOsPid` field (already within the per-active-session-heartbeat domain).',
    },
    {
