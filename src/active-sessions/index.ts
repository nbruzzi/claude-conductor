...
/**
 * `isOsPidAlive` — the same-host `kill(pid, 0)` liveness probe — was lifted to
 * the shared leaf `shared/os-pid.ts` (Lane B unify, 2026-06-07) so the read-only
 * `cohort-sight` board can consume it without importing this heavyweight module.
 * Re-exported here to preserve the existing import surface: `reconcile-boot`
 * imports it from this barrel, and the dotfiles wildcard shim re-mirrors it. See
 * `shared/os-pid.ts` for the full ESRCH-vs-EPERM / subtract-only-protect rationale.
 */
export { isOsPidAlive } from "../shared/os-pid.ts";
