...
// ============================================================================
// Memory-index filenames — CROSS-REPO convention (tiered-memory-index, PR-0).
//
// The tiered-memory-index slice splits the always-loaded memory index into two
// files in the shared memory dir: `MEMORY.md` (the hot subset, harness-loaded)
// and `MEMORY-FULL.md` (the complete index, read on-demand). Any consumer that
// enumerates the memory dir or classifies a memory-dir path by basename MUST
// exclude BOTH so the index files are never scored/archived/parsed as memos.
//
// This filename SET is a string convention shared across two repos
// (claude-conductor + claude-dotfiles), NOT a shared symbol — the dotfiles
// substrate cannot be imported here (cross-edge). Conductor therefore keeps its
// OWN `isIndexFile` mirroring the dotfiles leaf `src/shared/memory-index-shared.ts`
// (same name, same two-filename semantics). The literal-divergence between the
// two repos' one-off filters is exactly the bug the REV-5 census found (three
// audit rounds of dotfiles-scoped greps missed the conductor consumers because
// they keyed on different literals). The pinned set + each-repo unit test is the
// pragmatic contract. See docs/conventions/memory-index-filenames.md.
// ============================================================================

/** The always-loaded hot memory index (harness-injected at SessionStart). */
export const MEMORY_FILE = "MEMORY.md";
/** The complete memory index (read on-demand; generated by regen-memory-index). */
export const MEMORY_FULL_FILE = "MEMORY-FULL.md";

/**
 * True iff `name` is one of the memory-index files (`MEMORY.md` /
 * `MEMORY-FULL.md`). `name` is a basename WITH its `.md` extension (e.g. a
 * `readdirSync` entry, or `basename(path)`); callers that hold an
 * extension-stripped stem must compare on the un-stripped basename. Use this
 * at every memory-dir enumerator/classifier instead of a bare
 * `name === "MEMORY.md"` so a new index file is never missed (the REV-5
 * recurring-miss class).
 */
export function isIndexFile(name: string): boolean {
  return name === MEMORY_FILE || name === MEMORY_FULL_FILE;
}
