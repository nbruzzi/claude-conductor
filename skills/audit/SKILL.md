...
- **Recurring finding-class → ONE dedicated exhaustive pass, not another general round.** When the SAME finding-class surfaces across ≥2 rounds (round 1 catches instance A, round 2 catches B, round 3 catches C…), that recurrence IS the diagnosis: **the sweep was never exhaustive** — general auditors catch instances incrementally, each running its own partial sweep. Do NOT add another general round (the aggregate plateaus while the real set stays open). Instead commission ONE auditor with a single, narrow, **exhaustive** mandate scoped to that finding-class. **The FORM of the exhaustive pass is class-dependent — pick the form, don't default to one:** missed members of a set (consumers / call-sites / env vars / platforms / repos) → exhaustive **census/enumeration**; missed edge cases in one function → exhaustive **boundary/case sweep**; a premise keeps proving false somewhere new → exhaustive **premise-trace across all dependents**; the same race/state-bug keeps reappearing → exhaustive **interleaving/state trace**. **Discipline regardless of form:** multi-strategy union'd search (a single grep/lens is what kept missing items); span **all surfaces/repos** keyed on the **artifact/concept, not per-repo literals** (a literal-anchored grep structurally cannot see a cross-repo consumer that spells the same concept differently); primary-source-verified; end with an explicit completeness-confidence statement naming residual untraceable cases. For an "introduce a new artifact into a shared namespace" change, run the exhaustive pass as part of the _plan_, not a late round. It doubles as a premise check — an exhaustive pass can falsify a "single-repo / cross-edge: NONE / fully-handled" claim. See `feedback-recurring-finding-class-exhaustive-pass` (host memory).
