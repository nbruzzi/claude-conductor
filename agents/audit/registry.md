...
The audit board is a catalog of 16 expert auditors — 13 cold (pure domain expertise, no project context) and 4 familiar (project-aware, get memory context injected at commission time) with 1 template (`familiar/_template.md`, unregistered). The `/audit` skill reads this registry to match auditors to plans.
