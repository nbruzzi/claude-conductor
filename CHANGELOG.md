...
- **Phase 4 Step A Layer 1 — `peer-message-deliverer` hook + cursor 2PC + body fencing** (in progress; plan `~/.claude/plans/eventual-marinating-wall.md` v5 §Phase 1). New `user-prompt-submit` hook (position 1, before `teammate-idle-reminder`) that surfaces new peer messages across all channels where this session has a NATO identity claim. Closes backlog L133 (parallel coordination — no cross-window delivery). **Two-phase cursor commit** at `<channel-dir>/peer-message-emit-cursors/<sid>.json` (sibling-shape to `LastSeenCursor`): emit-turn writes `<sid>.json.pending`; next fire promotes pending → committed via atomic rename. Silent loss impossible because cursor advance gates on operator-reaches-next-prompt (consumption evidence). **Body fencing + sanitization** (MAJOR-1 fold + MINOR-3 refinement from Bravo sibling cross-audit on plan v2/v3): targeted-pattern strip (system-reminder/function_calls/antml-namespace/fence-marker/bare-`</`) replaces each with `[redacted-platform-marker]`; bare-`<` escape via `&lt;`; per-emission UUID-nonce fence wrap; 200-char inline truncate with body_ref overflow. Multibyte UTF-8 preserved verbatim (em-dashes / smart quotes / emoji / ellipsis NOT stripped). **Emission cap** 50/prompt aggregate across channels; per-channel overflow → summary-mode block with cursor advance to newest-suppressed mtime (full batch readable via `channels read --since-cursor`). **Recovery verbs:** `channels show-message-cursor <id>` (committed + pending state) + `channels forget-message-cursor <id>` (clears both). Fail-open + breadcrumb failure-mode class (any thrown helper → single outer `pass()`; never breaks UserPromptSubmit chain). Substrate shipped at `d44fa14` (47/47 tests). Hook: 25/25 tests. Atomic wiring: `bundled-check-names.ts` + `bundled-registrations.ts` + `lock-domain.ts` (new row with `["per-channel-cursor", "presence-failure-log"]` domains) + `bundled-registrations.test.ts` `EXPECTED_COUNT` 11→12 + `package.json` 2 new exports map entries. NEW doc `docs/architecture/inter-sibling-communication-layers.md` — 4-layer model permanent home. Decision K in `decisions/phase-4.md`.
