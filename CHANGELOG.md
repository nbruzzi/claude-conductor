...
Phase 4 Step A's Bravo lane (B1 + B2) ships substrate primitives for the inter-sibling communication arc decomposed into 4 layers per plan v5 `~/.claude/plans/eventual-marinating-wall.md`. Layer 1 (push delivery / `peer-message-deliverer` hook) is Alpha's lane — paused this session per Nick's directive; lands in a follow-up. Layer 2 (NATO identity + roles) was already shipped in Phase 1 Slice 5. Layers 3 + 4 land here.
...
- B1 PR #41 (Phase 0 SSOT + Layer 3 walkie-talkie + Layer 3 v5 atomicity + Decision I, on branch `phase-4-step-a-b1-layer-3`) — 5/5 CI runs green (4 push-event + 1 pull_request-event); Alpha sibling cross-audit verdict SHIP. Final merge-commit SHA filled at merge time per the sibling `Phase 3 Step F` cite precedent (pre-merge branch SHAs orphan under squash-merge policy; cite-the-merge-SHA-not-the-branch-internal-SHA is robust per `feedback-merge-policy-per-pr-choice.md`).
- B2 (Layer 4 `digest` + convention + Decision J) on branch `phase-4-step-a-b2-layer-4` via staged-diff KS + CLI DX fold + Alpha sibling cross-audit fold. Pipeline clean: typecheck + format + lint + tests; 5 audit cycles on this work (KS + CLI DX staged-diff + Alpha sibling MAJOR-1 SHA-correctness + 3 distinct-lens MINORs + MAJOR-2 SHA-recurrence catch on the fold recheck). Final merge-commit SHA + PR # filled at merge time per the sibling `Phase 3 Step F` precedent in this CHANGELOG (cite-the-merge-SHA-not-the-branch-internal-SHA is robust against `--amend` orphans + squash-merge orphans).
- 5 audit cycles + 1 plan-level catch lineage captured in PR #41 body + memory candidate `substrate-precedent-as-design-rescue` filed for post-arc wind-down.

#### Deferred (Phase 4 Step B / B1.b / follow-up arcs)

- **Layer 1 (`peer-message-deliverer` hook + cursor substrate + recovery verbs + dotfiles paired PR #A2)** — Alpha's lane; paused this session per Nick's directive.
- **SessionStart-driven reaper for departed-peer auto-out** — replaces the v4 Stop-hook auto-out draft that was dropped per per-turn-Stop precedent. New backlog candidate (Phase 4 Step B).
- **`listLivePeers({excludeOut: true})` flag** + **`channels-gc-reaper` extension for `peer-message-emit-cursors/`** — plan-listed for B1 but legitimately deferred (no in-tree consumer yet; gc-reaper extension requires Alpha's A1 substrate first). Follow-up.
- Minor CLI DX polish from B2 staged-diff audit (worked example for digest in convention doc + CLI file-header `Usage:` block hygiene + top-level help glosses + optional `verification_budget_consumed_ms` rename) — deferred to follow-up.
  > > > > > > > 2011c7c (feat(channels): `digest` kind + parseDigestBody shared parser + verification-budget convention (Phase 4 Step A Layer 4 / B2))
