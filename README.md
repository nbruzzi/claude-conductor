...
- **NOT an AI code-review tool.** The audit-loop pattern (NIT / OBS / BLOCKER classification + adversarial review + fold-to-fixed-point + multi-lens convergence) is pre-PR **plan-tier convergence among sibling Claude sessions writing code together** — not post-PR review-substitution. AI code-review tools (CodeRabbit, Reviewable, Greptile, Devin's PR review) operate on the post-PR human-review surface. `claude-conductor` operates on a different surface: plan-tier audit-ask + send-time `audit-verdict` schema validation + multi-NATO convergent ratify + Stop-hook + PR-tier human ratify. The two surfaces compose (pre-PR cohort discipline + post-PR review tool) but do not substitute for each other.
