...
#
# Static check: every tracked source file carries an SPDX license header
# near the top. Closes the R-3 enforcement gap for the "Apache-2.0 SPDX
# header on every new source file" convention (CONTRIBUTING.md) — which was
# previously convention-by-vigilance only. ESLint has NO SPDX rule and lints
# .ts only (eslint.config.js files-glob), so .sh/.js/.mjs/.cjs were entirely
# ungated; this gate is the cross-file-type closure.
#
# Detects:
#   SPDX-001 — source file missing 'SPDX-License-Identifier' within its
#              first 5 lines (a shebang on line 1 is allowed; the SPDX
#              marker just has to appear in the header region).
#
# Source file types scanned (tracked only): .ts .sh .js .mjs .cjs
...
# depth, so '*.ts' covers src/**, test/**, scripts/**, etc.
SOURCE_PATHSPECS=(
  "*.ts"
  "*.sh"
  "*.js"
...
# --- 4. Check each file for an SPDX header within its first 5 lines ---
# First-5-lines window allows a shebang on line 1 (shebang precedes SPDX on
# CLI scripts) while keeping the header near the top. Grep for the
# 'SPDX-License-Identifier' marker (presence); canonical form is
# 'SPDX-License-Identifier: Apache-2.0'. The grep runs inside an `if !`
# condition, so its no-match exit (1) does NOT trip `set -e`.
MISSING=()
for f in "${FILES[@]}"; do
...
VMSG="missing SPDX-License-Identifier header — add '// SPDX-License-Identifier: Apache-2.0' (or '# ...' for shell) within the first 5 lines; a shebang stays on line 1"
for f in "${MISSING[@]}"; do
