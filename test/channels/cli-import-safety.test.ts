...
describe("cli DieContext per-invocation isolation (RE-W2-3)", () => {
  it("Matrix: --json then plain — second call emits plain stderr (no leak)", async () => {
    ...
    try {
      ...
      //
      // Post-Slice-8.5: when process.exit is mocked, die() throws a
      // `DieAlreadyHandled` sentinel after stderr is written, and
      // runChannelsCli's catch-all re-throws the sentinel rather than
      // re-firing die() with category=UNCAUGHT (RE-W2-6 fix). The test
      // catches the sentinel so we can assert on the stderr output that
      // happened BEFORE the sentinel throw.
      stderr = "";
      try {
        ...
        if (!(err instanceof Error) || err.name !== "DieAlreadyHandled") {
          throw err;
        }
      }
