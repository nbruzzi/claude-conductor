...
The 4 lineage-bearing surfaces are touched by distinct authoring lanes — substrate type (`lineage-envelope.ts`), audit-verdict body (`audit-verdict.ts`), strict-frontmatter parsers (`handoff-body-parser.ts` + `memory-frontmatter-parser.ts`), and skill docs (`commands/session/handoff.md` + `commands/session/handoff-resume.md`). The Cycle 1 audit cycle surfaced three discipline instances where an authoring lane risked drifting from the substrate-canonical truth — all three caught + folded inline pre-merge by the cohort's preemptive-fold-on-OBS discipline (Charlie cross-pair-shadow `5516484a` + self-correction `cc46acf3` ratifying the discipline-as-actual-mechanism):

1. **substrate-required vs convention-required conflation** — PR-A8 Step 3.5 originally annotated `produced_at` as "Required by the envelope spec"; substrate has `produced_at?: string | null` (optional). Convention requires it for handoffs only. The annotation now clarifies "Optional per substrate (`?: string | null`), but required for handoffs as a cross-surface convention". (Bravo `e3d89316` NIT-1; folded `420ef9c`.)
2. **field-ordering across emit sections** — Step 3.5 list, Step 4 template, and test helpers all match the substrate type ordering at `lineage-envelope.ts:78-87`: `kind_version → producer_session_id → produced_at → input_body_refs → input_handoffs → prompt_sha → model → cost`. YAML object keys are not parser-order-sensitive, but cross-section consistency matters for skill-doc readability and audit cross-referencing. (Bravo `e3d89316` NIT-2; folded `420ef9c`.)
...
**Cohort mechanism:** the preemptive-fold-on-OBS discipline (ceiling-standard "no known gaps" + `[[feedback-substrate-fix-pattern-must-self-mirror]]`) closes these instances as audits surface them. All 3 instances above were folded inline pre-merge rather than landing as technical debt — the cohort doesn't accrue a "rule-of-N codification window" in practice. Audit recommendation: when reviewing a PR touching any lineage-bearing surface, lens-check substrate fidelity as a Contract-class lens dimension; primary-source-verify the substrate type definition before extending OR codifying a NIT-class observation about it (per Charlie self-correction `cc46acf3` 2026-05-27 — "when extending or codifying another NATO's NIT, primary-source-verify the cited current state").
