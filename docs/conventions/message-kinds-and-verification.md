...
### Phase 4.5 dashboard limited-mutation cycle 2026-06-04 — nudge (1 kind)

| Kind    | Semantic                                                                                                                                                                                                                                                                                                                                                | Recommended body                                                                                                                                                                                                                                                 |
| ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `nudge` | Directive wake-signal posted by the dashboard's Nudge / Check-comms write actions to wake a hung/idle peer — the substrate equivalent of Nick's manual "type in the sibling's terminal to wake them." A NEW kind, NOT free-form `note`/`question`/`status`: a nudge is a directive (wake / check-comms), not informational and not expecting an answer. | Free-form prose; the two modes differ only in body text — Nudge = `working?` (wake to re-check), Check-comms = `check your channel — directed from dashboard` (wake + actively poll `channels read --since-cursor`). No schema; ≤200 chars (dashboard-enforced). |

`nudge` has no body schema (free-form, like `note`). It joins the
urgent-kinds set sibling Monitor wake-filters honor (`question` /
`blocker` / `emergency` / `nudge`) — convention-level today (there is no
code urgent-kinds set); a session arming a Monitor includes `nudge` in
its wake-filter regex so a dashboard nudge from a non-partner still wakes
the target. Consumer: claude-conductor-dashboard Phase 4.5 (spec v2.1
§17.13) via `sendChannelMessage` → `appendMessage` (N1b).
...
| `nudge`                                      | Trust verbatim. A directive wake-signal (protocol state, not an assertion) — the reader's job is to WAKE and act on the directive (re-read the channel / confirm working), not to verify a factual claim. No schema.                                                                                                                                                                                                                                                                               |
